            ...
            <div class="box box-info">
                <form class="form-horizontal" action="{{URL::to('user/'.$user->id)}}" method="post" enctype="multipart/form-data">
                            ...
                            <div class="col-sm-9">
                                <select class="form-control select2" multiple="multiple" name="role_id[]" style="width: 100%">
                                    @foreach($roles as $role)
                                        <option value="{{$role->id}}" @if(in_array($role->display_name,$display_names)) selected @endif>{{$role->display_name}}</option>
                                    @endforeach
                                </select>
                    ...
                    <div class="box-footer">
                        <a href="{{route('user.index')}}" class="btn btn-default">返 回</a>
